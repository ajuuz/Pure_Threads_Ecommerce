...
const Orders = () => {
  ...
  useEffect(()=>{
    const fetchOrders = async ()=>{
      ...
      const ordersResult = await getOrders(sortCriteria,currentPage,limit,tab);
    ...
    <div className="min-h-screen pe-8">
      <div className="max-w-3xl mx-auto">
        <div className='flex justify-between'>
          <h1 className="text-3xl text-nowrap font-bold text-gray-900 mb-8">My Orders</h1>
          <Button onClick={tab==="successOrders"?()=>setTab("failedOrders"):()=>setTab("successOrders")} className="m-0 w-fit">{tab==="successOrders"?"Go to your Payment Failed Orders":"Go Back to Orders"}</Button>
        </div>
        <div className="space-y-6  ">
          {orders.map((order,index) => (
              <OrderCard order={order} setOrders={setOrders} index={index} handleCancelOrder={handleCancelOrder} handleReturnOrder={handleReturnOrder}/>
          ))}
        </div>
